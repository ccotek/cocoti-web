...
export default function PricingSection({ pricing, locale = 'fr' }: PricingSectionProps) {
  ...
  // URL du dashboard pour redirection
  const dashboardUrl = `${process.env.NEXT_PUBLIC_DASHBOARD_URL || 'https://app.cocoti.sn'}/${locale}`;

  return (
    <section id="pricing" className="section-padding bg-white">
      <div className="container space-y-10">
        <motion.div className="max-w-2xl space-y-4" {...fadeInUp}>
          <h2 className="text-3xl font-bold sm:text-4xl">{pricing.title}</h2>
        </motion.div>
              ...
              return (
                <motion.div
                  ...
                  className={`relative bg-white rounded-2xl shadow-lg overflow-hidden transition-all hover:shadow-xl ${
                    plan.highlight ? 'scale-105 border-2 border-magenta' : 'border border-cloud'
                  }`}
                >
                  {/* Titre du plan */}
